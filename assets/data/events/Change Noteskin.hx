function onEvent(e) {
    if (e.event.name == "Change Noteskin") {
        for (a in strumLines.members[e.event.params[1]]) updateStrumSkin(a, e.event.params[0] == "" ? "game/notes/TGCI" : e.event.params[0]);
        for (a in strumLines.members[e.event.params[1]].notes) updateNoteSkin(a, e.event.params[0] == "" ? "game/notes/TGCI" : e.event.params[0]);
    }
}
